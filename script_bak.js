...
async function loadAndDisplayAbsatzData(einheitIdToLoad, tabId) {
    ...
    currentTabUserInputs[tabId] = {}; // Initialisiere/resette User-Inputs für diesen Tab
    ...
    try {
        ...
        // Datenaufbereitung für AnzeigeBedingung, IstGrundvoraussetzung UND KonklusiveAntwortenInfo
        if (currentAbsatzData.parameterListe) {
            currentAbsatzData.parameterListe.forEach(param => {
                // Bestehende Logik für AnzeigeBedingungJSON
                if (param.AnzeigeBedingungJSON) {
                    try {
                        param.Anzeige_Bedingung_Parsed = JSON.parse(param.AnzeigeBedingungJSON);
                    } catch (e) {
                        console.error(`Fehler beim Parsen von AnzeigeBedingungJSON für Parameter ${param.Parameter_ID}:`, e, param.AnzeigeBedingungJSON);
                ...
                // ### HIER IST DIE FEHLENDE ERGÄNZUNG ###
                // Logik für KonklusiveAntwortenInfoJSON hinzufügen
                if (param.KonklusiveAntwortenInfoJSON) {
                    try {
                        param.Konklusive_Antworten_Info = JSON.parse(param.KonklusiveAntwortenInfoJSON);
                    } catch (e) {
                ...
                // Bestehende Logik für IstGrundvoraussetzung
                param.Ist_Grundvoraussetzung_Bool = !!parseInt(param.IstGrundvoraussetzung); // Konvertiert '0' zu false, '1' zu true, null zu false
            });

            // Debugging-Log, um das Ergebnis zu prüfen (aus meinem vorherigen Vorschlag)
            console.log("DEBUG: Parameter-Daten nach Aufbereitung:", JSON.stringify(currentAbsatzData.parameterListe, null, 2));
        }

        if (typeof uiRenderNormInfo === "function") uiRenderNormInfo(currentAbsatzData.simulationsEinheit, tabId);
        
        // uiRenderParameters erstellt das HTML für ALLE Parameter
        if (typeof uiRenderParameters === "function") uiRenderParameters(currentAbsatzData.parameterListe, tabId, () => handleInputChange(tabId)); 
        
        if (typeof uiRenderActionButton === "function") {
             uiRenderActionButton(
                currentAbsatzData.parameterListe,
                tabId,
                () => processSimulationCurrentTab(tabId, true) 
            );
        }
        
        initializeProgress(tabId); // Initialisiert Fortschritt

        // Initiale Sichtbarkeit aller Parameter setzen
        updateParametersVisibility(tabId);

    } catch (error) {
        console.error('SimONA Logik: Fehler beim Laden oder Anzeigen der Absatzdaten für', einheitIdToLoad, ':', error.message, error.stack);
        if (typeof uiShowErrorInNormInfo === "function") uiShowErrorInNormInfo(`Fehler: ${error.message}`, tabId);
        if (typeof uiUpdateProgressBar === "function") uiUpdateProgressBar(tabId, 0, 'Fehler beim Laden der Daten.', true);
    }
}
