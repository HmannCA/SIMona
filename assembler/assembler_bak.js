...
function generateSqlForP2Data() { 
    ...
    function generateAllSql() {
        console.log("generateAllSql aufgerufen");
        let allSqlCommands = "";
        ...
        if (sqlP1.trim() === "" && sqlP2.trim() === "" && sqlP4.trim() === "" && sqlP3.trim() === "" &&
            sqlP1.includes("noch nicht implementiert") && sqlP2.includes("noch nicht implementiert") &&
            sqlP3.includes("noch nicht implementiert") && sqlP4.includes("noch nicht implementiert")) {
            alert("Keine Daten oder Funktionen für die SQL-Generierung vorhanden/implementiert. Bitte führen Sie die Prompts P1-P4 aus und fügen Sie die Antworten ein oder implementieren Sie die SQL-Generierungsfunktionen.");
            return;
        }
        
        allSqlCommands += `START TRANSACTION;\n\n`; // Einmal am Anfang für alle zusammen

        if (sqlP1 && !sqlP1.includes("noch nicht implementiert")) allSqlCommands += sqlP1.replace(/START TRANSACTION;\n\n|COMMIT;\n/g, ''); // Transaktionen entfernen
        if (sqlP2 && !sqlP2.includes("noch nicht implementiert")) allSqlCommands += sqlP2.replace(/START TRANSACTION;\n\n|COMMIT;\n/g, '');
        if (sqlP4 && !sqlP4.includes("noch nicht implementiert")) allSqlCommands += sqlP4.replace(/START TRANSACTION;\n\n|COMMIT;\n/g, '');
        if (sqlP3 && !sqlP3.includes("noch nicht implementiert")) allSqlCommands += sqlP3.replace(/START TRANSACTION;\n\n|COMMIT;\n/g, '');
        
        allSqlCommands += `\nCOMMIT;\n`; // Einmal am Ende

        displayGeneratedSql(allSqlCommands.trim(), 'gesamtesSqlOutput', false); // Alles zusammen überschreiben
        alert("Alle verfügbaren SQL-Befehle wurden generiert.");
    }
